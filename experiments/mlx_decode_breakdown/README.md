...
We measure per-codec-frame decode times of OSS TTS models ranging from 0.6B (Qwen3) to 8B (CSM/Miso) parameters. This is broken down by semantic backbone (an autoregressive LLM predicting semantic codes) and audio decoder (a smaller model predicting the remaining audio codes).
